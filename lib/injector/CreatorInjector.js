...
CI.inject = function(Type, resolvedDeps) {
  ...
  if (this._targetInitializer) {
    ...
    if (typeof initializer !== 'function') {
      throw new ResolutionError("Initializer function not found: " + this._targetInitializer);
    }
    initializer.apply(instance, resolvedDeps);

  } else {
    if (resolvedDeps.length > 0) {
      throw new ResolutionError("Initializer function not specified, but dependencies supplied");
    }
  }
